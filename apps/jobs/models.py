...
#     user = models.ForeignKey(User, related_name="commenter", on_delete=models.DO_NOTHING)
#     body = models.TextField()
#     created_at = models.DateTimeField(auto_now_add=True)
#     updated_at = models.DateTimeField(auto_now=True)

class BlogManager(models.Manager):
    def validate_blog(self, postData):
        errors = {}
        if len(postData['title']) < 4:
            errors['title'] = "Your blog title needs to be four or more characters"
        if len(postData['title']) > 100:
            errors['title'] = "Your blog title needs to 100 or less characters"
        if len(postData['subtitle']) < 4:
            errors['subtitle'] = "Your blog subtitle needs to be four or more characters"
        if len(postData['subtitle']) > 50:
            errors['subtitle'] = "Your blog title needs to 50 or less characters"
        if len(postData['slug']) < 4:
            errors['slug'] = "Your slug needs to be four or more characters"
        if len(postData['slug']) > 50:
            errors['subtitle'] = "Your slug can be no more than 50 characters"
        if len(postData['body']) < 100:
            errors['body'] = "Your blog body needs to be at least 100 characters"
        return errors
...
class Blog(models.Model):
    ...
    image1 = models.ImageField(default=0)
    image2 = models.ImageField(default=0)
    body = models.TextField()
    user = models.ForeignKey(User, related_name="blogger", on_delete=models.DO_NOTHING)
    blog_category = models.ForeignKey(BlogCategory, related_name="blog_cat", on_delete=models.DO_NOTHING)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    objects = BlogManager()